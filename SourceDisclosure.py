...
def initArguments():
    parser = argparse.ArgumentParser(
        ...
        epilog='''
Examples: 
    1. For a single url('http://bing.com') to scan: 
        ```
        python SourceDisclosure.py --url=http://bing.com --scale=tiny --threads=4 --timeout=8 
        ```
    2. For a bunch of urls in file('url.txt') to scan: 
        ```
        python SourceDisclosure.py --urls=url.txt --threads=4 --timeout=8 --level=INFO 
        ```
        '''
    )
